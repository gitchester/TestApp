...
public class QueuedMuxer {
    ...
    private long lastAudioTimeFrame;
    private long lastVideoTimeFrame;
    ...
    public void writeSampleData(SampleType sampleType, ByteBuffer byteBuf, MediaCodec.BufferInfo bufferInfo) {
        if (mStarted) {
            long lastTimeFrame = getLastTimeFrameForSampleType(sampleType);
            if (bufferInfo.presentationTimeUs < lastTimeFrame) {
                bufferInfo.presentationTimeUs = lastTimeFrame + 1;
            }
            Logger.getAnonymousLogger().info("chuong-SlowMotion " + sampleType.toString() + " presentationTimeUs==" + bufferInfo.presentationTimeUs);
            mMuxer.writeSampleData(getTrackIndexForSampleType(sampleType), byteBuf, bufferInfo);
            setLastTimeFrameForSampleType(sampleType, bufferInfo.presentationTimeUs);
            return;
        }
    ...
    private long getLastTimeFrameForSampleType(SampleType sampleType) {
        switch (sampleType) {
            case VIDEO:
                return lastVideoTimeFrame;
            case AUDIO:
                return lastAudioTimeFrame;
            default:
                throw new AssertionError();
        }
    }

    private void setLastTimeFrameForSampleType(SampleType sampleType, long pt) {
        switch (sampleType) {
            case VIDEO:
                lastVideoTimeFrame = pt;
                break;
            case AUDIO:
                lastAudioTimeFrame = pt;
                break;
            default:
                throw new AssertionError();
        }

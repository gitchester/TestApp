 ...
 */
package com.sample.home.testapp.slowmotion.test.androidtranscoder.engine;
...
import com.sample.home.testapp.slowmotion.test.androidtranscoder.format.MediaFormatStrategy;
import com.sample.home.testapp.slowmotion.test.androidtranscoder.utils.MediaExtractorUtils;
...
public class MediaTranscoderEngine {
    ...
    private void setupMetadata() throws IOException {
        ...
        try {
            mDurationUs = 32000000;//Long.parseLong(mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)) * 1000;
        } catch (NumberFormatException e) {
            mDurationUs = -1;
        }
    ...
    private void setupTrackTranscoders(MediaFormatStrategy formatStrategy) {
        ...
        if (audioOutputFormat == null) {
            Logger.getAnonymousLogger().info("audio output format is null");
    ...
    private void runPipelines() {
        ...
        while (!(mVideoTrackTranscoder.isFinished() && mAudioTrackTranscoder.isFinished())) {
            boolean isVideoBusy = mVideoTrackTranscoder.stepPipeline();
            boolean isAudioBusy = mAudioTrackTranscoder.stepPipeline();
            boolean stepped = isVideoBusy || isAudioBusy;
            loopCount++;
            Logger.getAnonymousLogger().info("chuong-SlowMotion ENGINE loopCount==" + loopCount);
    ...
    public static class PTSObject {

        public long presentationTimeUs;
        public long numberOfFrames;
        public List<Long> videoPresentationTimeUs;

        public PTSObject() {
            videoPresentationTimeUs = new ArrayList<>();
        }

...
import com.sample.home.testapp.slowmotion.test.androidtranscoder.compat.MediaCodecBufferCompatWrapper;
...
class AudioChannel {
    ...
    public boolean feedEncoder(long timeoutUs) {
        ...
        long presentationTimeUs = remixAndMaybeFillOverflow(inBuffer, outBuffer);
        mEncoder.queueInputBuffer(encoderInBuffIndex,
                0, outBuffer.position() * BYTES_PER_SHORT,
                presentationTimeUs, 0);
        if (inBuffer != null) {
    ...
    private long remixAndMaybeFillOverflow(final AudioBuffer input,
        ...
        if (inBuff.remaining() > outBuff.remaining()) {
            Logger.getAnonymousLogger().info("chuong-SlowMotion remixAndMaybeFillOverflow presentationTimeUs==" + input.presentationTimeUs);
            // Overflow

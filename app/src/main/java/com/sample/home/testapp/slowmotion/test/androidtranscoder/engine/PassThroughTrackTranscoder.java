...
public class PassThroughTrackTranscoder implements TrackTranscoder {
    ...
    public boolean stepPipeline() {
        ...
        long pt = mExtractor.getSampleTime();
        if (mExtractor.getSampleTime() >= startPoint) {
            if (mExtractor.getSampleTime() <= endPoint) {
                pt = (mExtractor.getSampleTime() - startPoint) * speed + startPoint;
            } else {
                pt = (endPoint - startPoint) * speed + startPoint + mExtractor.getSampleTime() - endPoint;
            }
        }

//        mBufferInfo.set(0, sampleSize, mExtractor.getSampleTime(), flags);
        mBufferInfo.set(0, sampleSize, pt, flags);
        mMuxer.writeSampleData(mSampleType, mBuffer, mBufferInfo);

...
import com.sample.home.testapp.slowmotion.test.androidtranscoder.format.MediaFormatExtraConstants;
...
public class VideoTrackTranscoder implements TrackTranscoder {
    ...
    private ByteBuffer mWrittenByteBuffer;

    private int mVideoEncodedFrameCount = 0;
    private int mVideoDrainedEncodedCount = 0;
        ...
        if (sampleSize > 0) {
            ...
            if (presentationTimeUs >= startPoint) {
                if (presentationTimeUs <= endPoint) {
                    presentationTimeUs = (presentationTimeUs - startPoint) * speed + startPoint;
                } else {
                    presentationTimeUs = (endPoint - startPoint) * speed + startPoint + presentationTimeUs - endPoint;
                }
            }
            mDecoder.queueInputBuffer(result, 0, sampleSize, presentationTimeUs, isKeyFrame ? MediaCodec.BUFFER_FLAG_SYNC_FRAME : 0);
        }
        mExtractor.advance();
